...
- **Definitieve cleanup (2026-08-15 ~14:30):** operator besloot dat de 3 resterende OOM-modellen (Ministral, Qwen3-30B, Step3-VL) nutteloos zijn — alle 3 verwijderd uit `config/models.yaml` + aliases + `~/.pi/agent/models.json`. Nu **15 lokale modellen** in models.yaml, **101 in `/v1/models`**. Focus operator ligt op `Huihui-gemma-4-26B-A4B-it-abliterated-Q8KV` (54 t/s) en `qwen3.8-27b` (64 t/s thinking / 17 t/s instruct) — beide live geverifieerd werkend (denk-branch + `OK` reply, `finish=stop`).
