...
- **LIVE-verificatie na restart (todo):** `guardianctl status` (cloud=True, opt-in=False), een cloud-request → raw WAL-event in `data/capture/guardian_capture_current.jsonl.gz` (gzip!), `guardianctl export --verify-checksums`, media-bestand bij image-request. **Leerstuk uit 2026-08-24:** na een code-deploy die de cache-shape verandert moet een catalog-refresh (`POST /api/cloud/catalog/refresh`) lopen.
