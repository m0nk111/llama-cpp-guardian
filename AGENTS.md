...
- **Legacy docs git rm'd → `.scratch/cleanup/legacy-docs/` (operator OK'd):** `docs/BENCHMARK_SUMMARY.md`, `CONTEXT_BENCHMARKS.md`, `REAL_BENCHMARK_RESULTS.md`, `unsloth_candidates.md`, `SERVER_UPGRADE_PLAN.md`, `HYDRO_CONTEXT_ARCHITECTURE.md`, `model_registry.json` (Feb-2026, unreferenced, superseded by `docs/MODEL_BENCHMARKS.md`). **Consequence:** `scripts/analyze_benchmark.py`, `benchmark_context.py`, `download_models_py.py` still point at those removed doc paths — these are legacy one-off scripts the operator chose to keep; their targets are gone (a known caveat, not a live-path regression).
- **Operator consciously did NOT** remove the one-off script set (`download_models_*.py/sh`, `needle_test.py`, `stress_test.py`, `cleanup_invalid_benchmarks.py`, `bench_qwen36_variants.py`) — they stay in `scripts/`.
- **Test suite:** 938 passed / 3 skipped. One **pre-existing** failure unrelated to cleanup: `tests/unit/test_server.py::test_cloud_attempts_resolve_google_full_address` (google cold-start fallback resolves upstream as `models/gemini-2.5-flash` but test expects `google/gemini-2.5-flash`) — fails on pristine HEAD too; a real but separate google-catalog fallback bug to fix in its own change. `data/bench-models/` (resumable state of active `scripts/bench_all_models.py`) and `data/model_finetune_v2_results.json` (default finetune output) intentionally kept in `data/`.
